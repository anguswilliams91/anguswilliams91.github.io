...
i.e., we *marginalise* out the parameters of the model $\theta$ to obtain the probability of the data $y$ given the model choice $M$, given our beliefs about the model parameters $p(\theta | M)$ (i.e., we do not choose a single set of model parameters, but consider all likely configurations of $\theta$).
This adds some intuition about the Bayes' factor: we choose the model for which the data $y$ are *most likely*, i.e. the model for which $y$ are most typical.
...
$$\int p(\theta | y, M) p(\theta | M) \, \mathrm{d}\theta = p(y | M).$$
...
* The new dataset is assumed to contain $N$ data points.
* $\tilde y$ is a new, unseen dataset that (i.e., is not part of the training data $y$).
* $p_t(\tilde y)$ is the true data generating distribution of $\tilde y$.
* $p(\tilde y | y, M)$ is the *predictive distribution* of the model being assessed, $M$ (i.e., the probability the model assigns to seeing $\tilde y$, given that the model has seen the training data $y$).

For Bayesian models, the predictive distribution is

$$p(\tilde y | y, M) = \int \mathrm{d}\theta \, p(\tilde y | \theta, M) p(\theta | y, M).$$

It is so called because this is the pdf we would use for predicting new data, because we include the knew information we got from the training data $y$.
...
\begin{equation}
\begin{aligned}
p(\tilde y | y, M_1) &\approx p(\tilde y | y, M_2)
\implies \mathrm{elpd}_{M_1} \approx \mathrm{elpd}_{M_2}.
\end{aligned}
\end{equation}
...
However, there is a problem - in order to evaluate the elpd, we require the *true data generating distribution*, $p_y(\tilde y_i)$.
...
1. The predictive distribution of $M$ with data point $i$ removed is approximately the same as when it is included: $p(\tilde y | y, M) \approx p(\tilde y | y_{-i}, M)$.
2. That the training set is drawn from the true data generating distribution $y \sim p_t(y)$.
...
This approach does not have the same problematic sensitivity to the prior as the Bayes factor approach, and is favoured by many now (indeed, it's implemented in [R](https://cran.r-project.org/web/packages/loo/index.html) and [python](https://arviz-devs.github.io/arviz/)).
Note that step (1) in the above recipe could be very expensive computationally (imagine carrying out MCMC $N$ times for a data set with 1000 points!)
Fortunately, [Vehtari et al. (2016)](https://arxiv.org/abs/1507.04544) provide an efficient approximation to this procedure that can alleviate this problem.
...
The paper proves that the marginal likelihood is equivalent to taking the average of *"leave-$p$-out cross validation averaged over all values of $p$ and all held-out test sets"*. 
In maths, this means:

$$\log p(y | M) = \sum_{p=1}^N S_\mathrm{CV}(y; p) \label{result}\tag{9}$$

where we have training data $y$ containing $N$ data points.
...
I really enjoyed seeing the result from [Fong & Holmes (2020)](https://academic.oup.com/biomet/article/107/2/489/5715611), because it really joined the dots between different concepts in model comparison that I have learned about.
